...
function addTaskList() {
    ...
    html += '</select>'
            + '</div>'
            +    '<div class="error"></div>'
            +    '<button class="pop-btn btn1" onclick="closePop()">取消</button>'
            +    '<button class="pop-btn btn2" onclick="confirmPop()">确定</button>';
...
function clickAllTaskList(ele) {
    ...
    for(var i = 0; i < nodes.length; i++) {
        if(hasClass(nodes[i], 'choose')) {
            removeClass(nodes[i], 'choose');
        }
    }
    if(ele) {
        addClass(ele, 'choose');
...
function editTask() {
    ...
    var nodeEditDetail = $('.t-content-detail .myTextArea');
    //$('.t-status .t-status-add').click = null;//新增任务按钮不起作用
    $.click($('.btn3')[1], modifyTask);//保存按钮重新绑定监听事件，区分新增任务保存
    //日期 编辑文本框数据清除并隐藏正常显示内容
...
function taskAdd() {
    ...
    if(hasClass(nodeCatAll, 'choose')) {//选中所有任务列表
...
function clickTaskStatus(ele) {
    ...
    for(var i = 0; i < parent.children.length; i++) {
        if(hasClass(parent.children[i], 'choose')) {
            removeClass(parent.children[i], 'choose');
        }
    }
    addClass(ele, 'choose');
        ...
        for(var j = 0; j < task.length; j++) {
            if(task[j].date === date[i]) {
                if(task[j].finish === true) {
                    html += '<li><h6 onclick="taskClick(this)" class="task-finish">';
                } else {
                    html += '<li><h6 onclick="taskClick(this)" class="task-unfinish">';
                }
                html += '<i class="icon-check"></i>' 
                + '<span class="detail-text">'
                +task[j].name+'</span>'
                +'<i class="delete icon-minus-circled" onclick="del(event)"></i>'
                +'</h6></li>';
                 if(s === false) {
...
function updateTaskList(nodeID) {
    ...
    for (var i = 0; i < cat1.length; i++) {
        ...
        // if(name != cat1[i].name) {
            html += '<li><h4 onclick="clickAllTaskList(this)">'; 
        // } else {
        ...
        + '(' + length + ')'; 
        if(i !== 0){
            html += '<i class="delete icon-minus-circled" onclick="del(event)"></i>';
        }
        html += '</h4><ul class="t-list-item">';
        for (var j = 0; j < cat2.length; j++) {
            if(cat1[i].id === cat2[j].father) {
                // if(name != cat2[j].name) {
                    html += '<li><h5 onclick="clickAllTaskList(this)">';
                // } else {
